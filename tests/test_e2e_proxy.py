...
class TestProxyCacheE2E(unittest.TestCase):
    ...
    def test_store_then_hit(self):
        ...
        request_hash = self._make_request_hash(
            "POST", "api.openai.com", "/v1/chat/completions", body
        )
    ...
    def test_different_host_different_key(self):
        ...
        # OpenAI should hit
        self.assertTrue(
            self.client.send({"cmd": "cached_api_check", "request_hash": hash_oa}).get("cached")
        )
        # Anthropic should miss (different host in key)
        self.assertFalse(
            self.client.send({"cmd": "cached_api_check", "request_hash": hash_an}).get("cached")
        )
