...
if not os.path.isdir(REPO):
    ...
    print("     Set the TOOLRECALL_REPO env var or symlink ~/toolrecall to your clone.")
    sys.exit(1)
...
check(
    f"All {files_read} unique files read from disk",
...
check(
    "Tokens did NOT increase on re-reads",
...
check(
    f"Hits increased to {hits_expected}",
...
check(f"Tokens increased by {new_tokens:,} for 3 new files", new_tokens > 0, "no increase")
...
for cmd in commands:
    ...
    check(f"Terminal: {cmd}", not r1.get("error", False), f"error: {r1.get('error')}")
...
print(
    f"     Terminal: {tc.get('hits', 0)} hits, {tc.get('misses', 0)} misses, {tc.get('tokens_read_from_disk', 0)} tokens"
)
...
print(
    f"     Code: {cc.get('hits', 0)} hits, {cc.get('misses', 0)} misses, {cc.get('tokens_read_from_disk', 0)} tokens"
)
...
for emoji, key in layers.items():
    ...
    if key != "mcp_cache" or entries > 0 or tokens > 0:
        print(
            f"  {emoji}  {key:<25} {entries:>4} entries | "
...
print(f"  {'─' * 57}")
print(
    f"  TOTAL: {final.get('file_cache_entries', 0) + final.get('terminal_cache_entries', 0) + final.get('script_cache_entries', 0) + final.get('code_cache_entries', 0) + final.get('mcp_cache_entries', 0):>4} entries, {total_tokens:>8,} tokens | "
    f"~${total_cost:.4f} @ $2/M"
)
...
_conn.close()
old_bug = (
    old_bug_file + total_tokens - (final.get("file_cache", {}).get("tokens_read_from_disk", 0))
)
print(
    f"     Alter Bug (alle hits):{old_bug:>10,} tokens = ${old_bug / 1_000_000 * 2:.4f} (×{old_bug / max(total_tokens, 1):.0f})"
)
