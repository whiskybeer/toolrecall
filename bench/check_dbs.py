...
for name, files in [("prefix", prefix_dbs), ("toolrecall", toolrecall_dbs)]:
    ...
    last = con.execute(
        "SELECT turn_index, request_tokens, prompt_tokens, status FROM turn_log ORDER BY turn_index DESC LIMIT 1"
    ).fetchone()
    print(
        f"{name}: {os.path.basename(db)} {turns} turns, last={last[0]}, req_tok={last[1]}, prov_tok={last[2]}, status={last[3]}"
    )
    rows = con.execute(
        "SELECT turn_index, request_tokens, prompt_tokens, ctx_dropped_tokens_cum FROM turn_log ORDER BY turn_index"
    ).fetchall()
    for r in rows:
