...
def build_accumulating_session(n_turns: int = 8, n_files: int = 3):
    ...
    snapshots = []
    msgs = [{"role": "system", "content": "You are a senior software engineer debugging a repo."}]

    msgs.append({"role": "user", "content": "Trace the bug. Read the relevant modules."})
    ...
    for t in range(2, n_turns + 1):
        msgs.append(
            {"role": "assistant", "content": f"Analysis {t}: re-inspecting files to verify flow."}
        )
        for fpath, content in files.items():
            msgs.append({"role": "tool", "content": content})  # RE-READ -> dup
        msgs.append({"role": "user", "content": f"Continue debugging, turn {t}."})
        snapshots.append(list(msgs))
    return snapshots, {}
...
def run_arm(url: str, snapshots: list, with_dedup: bool, iters: int) -> dict:
    ...
    for _ in range(iters):
        for snap in snapshots:
            if with_dedup:
                t0 = time.perf_counter_ns()
                payload, _ = dedup_messages(snap)
                hook_samples.append((time.perf_counter_ns() - t0) / 1000.0)
            else:
                payload, _ = snap, None
            path_samples.append(_post(url, {"model": "stub", "messages": payload}))
...
def main() -> None:
    ...
    out["WITH"] = {
        **summarize(arm_a["hook_samples"], "hook"),
    ...
    out["delta"] = {
        "path_p50_us_delta": round(d_p50, 2),
    ...
    print(
        f"WITH    (dedup enabled)    hook p50={out['WITH']['hook_p50_us']}µs "
        f"p99={out['WITH']['hook_p99_us']}µs | "
        f"path p50={out['WITH']['path_p50_us']}µs p99={out['WITH']['path_p99_us']}µs"
    )
    print(
        f"DELTA   path p50={out['delta']['path_p50_us_delta']}µs "
