...
def workload_bugfix(seed: int = 42) -> Workload:
    ...
    system_msg = [
        {"role": "system", "content": "You are a senior Python developer debugging a codebase."}
    ]
...
def workload_analysis(seed: int = 42) -> Workload:
    ...
    system_msg = [
        {"role": "system", "content": "You are a data analyst working with code metrics and logs."}
    ]
    ...
    for i in range(100):
        ...
        step(
            _make_msg(
                "Compare",
                f"{os.path.basename(files[0])} vs {os.path.basename(files[1])}",
...
def workload_review(seed: int = 42) -> Workload:
    ...
    for i in range(50):
        ...
        step(
            _make_msg("Daily review", f"core ({CORE_FILES[variation]})", questions[variation]),
            reads=core_paths,
            writes=[],
    ...
    for i in range(50):
        ...
        step(
            _make_msg("Final review", f"core ({CORE_FILES[variation]})", questions[variation]),
            reads=core_paths,
            writes=[],
            clean=core_paths,
        )
